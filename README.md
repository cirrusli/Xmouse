...
便携版也可以直接解压 `outputs\Xmouse-0.3.0-windows-x64.zip` 后运行 `Xmouse.exe`。请保留 `sqlite3.dll` 与程序在同一目录。
...
- 可在“剪贴板历史”设置页启用 Windows DPAPI；启用后，新记录按当前 Windows 用户加密。
...
Release 可执行文件位于 `target\release\xmouse.exe`。当前版本说明见 `docs\RELEASE-NOTES-0.3.0.md`，完整变更历史见 `CHANGELOG.md`。
