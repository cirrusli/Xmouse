...
Xmouse 0.12.0 是当前可用基线，已完成全屏游戏保护、钩子热路径优化、剪贴板类型/来源筛选、搜索高亮、选择后自动粘贴、个性化轨迹学习和有限安全动作绑定。下一版不以堆叠功能为目标，重点提升规则精度、键盘/无障碍体验和长期存储稳定性。
...
| GST-011 | 用户可为七条轨迹分别录制最多 3 份个人单笔样本，保存后立即参与最终识别和实时预测。 |
...
| GST-015 | 识别结果与执行动作分离；七条轨迹均可改绑有限安全动作或单独禁用。 |
...
- 32 项常规 Release 单元测试和 1 项真实剪贴板往返测试通过。
- 0.12.0 的 50.59 秒后台空闲基线为：CPU 增量 0、平均私有内存 2.27 MiB、平均工作集 14.86 MiB。
...
## 10. Xmouse 0.13.0 建议路线

### 10.1 推荐主题

**主题：在不增加常驻成本的前提下，补齐长期使用和可访问性。**

0.10–0.12 已完成本轮用户确认的游戏保护、剪贴板增强和有限动作绑定。0.13 建议以稳定性和细节质量为主，不进行 UI 框架整体迁移。
...
- 后台空闲 CPU、私有内存和钩子热路径不劣于 0.12.0 基线。
