...
Xmouse 0.4.0 是可用基线。0.5.0 不以堆叠功能为目标，而是重点提升冲突处理、可发现性、个性化和长期稳定性。

## 2. 产品概述

### 2.1 产品定位

Xmouse 是一款 Windows 原生轻量效率工具，通过鼠标右键或侧键手势完成窗口置顶、关闭页面、搜索选中文本、复制内容和打开剪贴板历史。
...
| 指标 | 目标 | 0.4.0 实测基线 |
| --- | --- | --- |
| 空闲 CPU | 平均 ≤ 0.1% | 60.86 秒内 CPU 增量低于本次计时分辨率 |
| 空闲私有内存 | ≤ 30 MiB | 平均 2.15 MiB，峰值 2.19 MiB |
| 空闲工作集 | 参考指标 | 平均 14.48 MiB，峰值 14.50 MiB |
| 后台线程 | 尽量固定且无忙等 | 稳定后 4 个线程 |
...
0.4.0 使用原生 HWND、GDI、自绘按钮、Common Controls v6、DWM 深色标题栏和 Per-Monitor V2 DPI。
...
代价是布局、焦点、键盘导航、无障碍和主题细节需要手动实现，因此下一版应优先拆分 UI 层，而不是继续扩大单文件窗口过程。
...
| 方案 | 优点 | 主要代价 | 当前结论 |
| --- | --- | --- | --- |
| Win32/GDI 自绘 | 资源最低、与现有架构直接兼容、部署简单 | 手工布局和无障碍成本高 | 0.5.0 继续使用，但必须模块化 |
| [WinUI 3](https://learn.microsoft.com/windows/apps/winui/winui3/) | 微软现代原生 UI、Fluent/XAML、控件完整 | Windows App SDK、XAML/C++ 或 C# 集成和部署复杂度 | 适合单独原型，不在 0.5.0 全量迁移 |
...
| [Slint](https://github.com/slint-ui/slint) | 声明式 UI、稳定 1.x API、多种渲染后端 | 新 DSL、运行/渲染层和许可证组合需评估 | 可做独立 UI 原型，不进入 0.5.0 主线 |
...
结论：0.5.0 保留 Win32/GDI 核心，先建立 `UiState → ViewModel → Win32 View` 分层。只有当 Winio 或 WinUI 3 原型在功能、体积、启动时间和内存上同时通过基线，才考虑 0.6.0 以后迁移设置窗口；鼠标钩子、动作和剪贴板后端无需随 UI 重写。
...
## 10. Xmouse 0.5.0 建议路线

### 10.1 推荐主题

**主题：更聪明地避免冲突，更快地找到历史，继续保持轻量。**

0.5.0 建议只交付下列 P0/P1 项，不进行 UI 框架整体迁移。
...
| P0 | 手势校准向导 | 降低“太容易触发”或“画不出来”问题 | L | 用户画 3–5 次样本，自动生成安全阈值，不直接暴露参数 |
| P0 | UI 模块化与无障碍 | 降低继续迭代的回归风险 | M | 拆分主窗口/历史/主题；Tab、焦点框、屏幕阅读器名称完整 |
| P0 | 存储迁移与维护 | 提升长期使用稳定性 | M | 数据库 schema 版本、WAL checkpoint、孤立媒体清理和恢复测试 |
| P1 | 收藏/固定历史 | 常用文本不会被淘汰 | M | 固定项置顶、默认不参与容量淘汰，可搜索和取消固定 |
...
校准不应重新暴露延时、距离、置信度等专业参数，而应让用户完成短流程：
...
- 数据库增加 `pinned` 和 `pinned_at` 字段。
- 固定项显示简洁图标，位于普通记录之前。
- 固定项默认不参与条数淘汰，但仍计入总磁盘上限。
- 提供可配置全局快捷键，建议默认 `Ctrl+Alt+V`，避免占用系统 `Win+V`。
- 0.5.0 仍保持“选择后只复制”；自动粘贴放到后续版本并默认关闭。
...
- `ui/history.rs`：历史弹窗与列表绘制。
- `ui/widgets.rs`：开关、分段按钮、卡片和提示条。
- `ui/view_model.rs`：可测试的显示状态与命令。

这一步不会改变技术框架，但能显著降低深色模式、DPI 和页面迭代时的连锁回归。
...
- 校准后短按误触低于默认配置，五个内置手势识别率不得下降。
- 固定历史在普通容量淘汰后仍存在，清空时需要单独确认是否包含固定项。
- 全局快捷键注册失败时不覆盖其他应用，界面显示冲突原因。
- 数据库从 0.4 schema 自动升级；失败时原数据库可恢复。
- 200 条历史下搜索输入、主题切换和弹窗打开无明显卡顿。
- 后台空闲 CPU、私有内存和线程数不劣于 0.4.0 基线的可接受范围。

## 13. 待确认产品决策

1. 0.5.0 是否将“按应用规则”作为首要功能。
