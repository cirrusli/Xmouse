...
Xmouse 0.5.0 是可用基线，已完成剪贴板置顶、数据库版本迁移和 UI 模块化。下一版不以堆叠功能为目标，而是重点提升应用冲突处理、可发现性、无障碍和长期稳定性。
...
| CLP-013 | 置顶项不参与普通容量淘汰；显式删除或清空包含置顶项时必须确认。 |
...
| 指标 | 目标 | 0.5.0 实测基线 |
| --- | --- | --- |
| 空闲 CPU | 平均 ≤ 0.1% | 60.90 秒平均 0.0225% |
| 空闲私有内存 | ≤ 30 MiB | 平均 2.16 MiB，峰值 2.20 MiB |
| 空闲工作集 | 参考指标 | 平均 14.41 MiB，峰值 14.43 MiB |
...
0.5.0 使用原生 HWND、GDI、自绘按钮、Common Controls v6、DWM 深色标题栏和 Per-Monitor V2 DPI。
...
代价是布局、焦点、键盘导航、无障碍和主题细节需要手动实现。0.5.0 已将 UI 拆分为主题、原生控件、通用组件、设置页、历史弹窗、历史行与格式化模块；下一版继续补足键盘焦点和无障碍信息。
...
| [WinUI 3](https://learn.microsoft.com/windows/apps/winui/winui3/) | 微软现代原生 UI、Fluent/XAML、控件完整 | Windows App SDK、XAML/C++ 或 C# 集成和部署复杂度 | 适合单独原型，不在 0.6.0 全量迁移 |
...
| [Slint](https://github.com/slint-ui/slint) | 声明式 UI、稳定 1.x API、多种渲染后端 | 新 DSL、运行/渲染层和许可证组合需评估 | 可做独立 UI 原型，不进入 0.6.0 主线 |
...
## 10. Xmouse 0.6.0 建议路线

### 10.1 推荐主题

**主题：更聪明地避免冲突，更快地找到历史，继续保持轻量。**

0.6.0 建议只交付下列 P0/P1 项，不进行 UI 框架整体迁移。
...
- 0.5.0 已完成 `pinned`、`pinned_at`、置顶排序和淘汰保护。
...
- `ui/history_popup.rs` 与 `ui/history_view.rs`：历史布局与列表绘制。
...
- 后台空闲 CPU、私有内存和线程数不劣于 0.5.0 基线的可接受范围。

## 13. 待确认产品决策

1. 0.6.0 是否将“按应用规则”作为首要功能。
